...
export default function Sidebar({ collapsed }: Props) {
  ...
  return (
    <aside className={clsx(
      'sticky top-0 hidden h-screen shrink-0 border-r border-slate-200/80 bg-white/90 backdrop-blur-xl lg:flex lg:flex-col transition-all duration-200',
      collapsed ? 'w-20' : 'w-72'
    )}>
      ...
      <div className={clsx('flex-1 overflow-y-auto', collapsed ? 'space-y-4 p-3' : 'space-y-5 p-4')}>
        <nav className="space-y-1.5">
          {!collapsed && <div className="px-2 pb-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Navigation</div>}
          {NAV_ITEMS.map(item => {
            ...
            return (
              ...
              >
                <span className={clsx('rounded-xl p-2 transition-colors', active ? 'bg-white/10' : 'bg-white shadow-sm ring-1 ring-slate-200')}>
                  {item.icon}
                </span>
        ...
        {!collapsed ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm text-black">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Watchlist</div>
                <div className="mt-1 text-sm font-medium text-slate-900">Logo view</div>
              </div>
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500">6 names</span>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-px overflow-hidden rounded-2xl bg-slate-200 p-px">
              {WATCHLIST.map(item => (
                <div
                  key={item.ticker}
                  className="group flex flex-col items-center gap-2 bg-slate-50 px-2 py-4 text-center transition-colors hover:bg-white"
                  title={item.name}
                >
                  <CompanyLogo ticker={item.ticker} name={item.name} size="sm" />
                  <div className="text-[10px] font-semibold uppercase tracking-wider text-black/60 group-hover:text-black">
                    {item.ticker}
                  </div>
                </div>
              ))}
